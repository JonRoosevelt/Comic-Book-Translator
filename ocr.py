...
class OCR:
    ...
    def scatter(self) -> any:
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                mode="markers",
                marker=dict(
                    size=4, color=self.dataframe["Density"], colorscale="bluered"
                ),
                text=self.dataframe["Density"],
    ...
    def recognize(self) -> any:
        ...
        try:
            for observation in request.results():
                ...
                self.data.append(
                    (observation.text(), observation.confidence(), [x, y, w, h])
                )
    ...
    def glue_sentences(self, x_threshold=0.05, y_threshold=0.02):
        words_with_coords = []
        if self.dataframe is None:
            return
        for i, word in enumerate(self.dataframe["Content"]):
            x_top_left = self.dataframe["x"][i]
            y_top_left = self.dataframe["y"][i]
            x_bottom_right = self.dataframe["Centroid x"][i] * 2 - x_top_left
            words_with_coords.append((word, x_top_left, y_top_left, x_bottom_right))

        # Sort words by their y-coordinate and then by their x-coordinate
        words_with_coords.sort(key=lambda w: (w[2], w[1]))

        sentences = []
        current_sentence = words_with_coords[0][0]
        last_x_right = words_with_coords[0][3]
        last_y = words_with_coords[0][2]

        for i in range(1, len(words_with_coords)):
            word, x_left, y_top, x_right = words_with_coords[i]
            x_gap = x_left - last_x_right
            y_gap = abs(y_top - last_y)
    ...
    def recognize_and_glue(self):
        self.recognize()
        glued_sentences = self.glue_sentences()
        return glued_sentences
